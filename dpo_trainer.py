...
class DPOTrainer(Trainer):
    ...
    def tokenize_row(self, feature):
        ...
        This method handles the truncation in case the prompt + chosen or prompt + rejected responses is/are too long. First
        ...
        if not self.is_encoder_decoder:
            ...
            prompt_tokens = self.tokenizer(feature["prompt"], add_special_tokens=False)
            prompt_tokens = {f"prompt_{k}": v for k, v in prompt_tokens.items()}

            if not isinstance(chosen, str):
                raise ValueError(f"chosen should be str but got {type(chosen)}")
            chosen_tokens = self.build_tokenized_answer(feature["prompt"], feature["chosen"])

            if not isinstance(rejected, str):
                raise ValueError(f"rejected should be str but got {type(rejected)}")
            rejected_tokens = self.build_tokenized_answer(feature["prompt"], feature["rejected"])

            # add BOS token at begining of the prompt
            ...
            for answer_tokens in [chosen_tokens, rejected_tokens, prompt_tokens]:
                if len(answer_tokens["prompt_input_ids"]) + longer_response > self.max_length:
                    if self.truncation_mode == "keep_start":
                        for i in ["prompt_input_ids", "prompt_attention_mask"]:
                            answer_tokens[i] = answer_tokens[i][: self.max_prompt_length]
                    elif self.truncation_mode == "keep_end":
                        for i in ["prompt_input_ids", "prompt_attention_mask"]:
                            answer_tokens[i] = answer_tokens[i][-self.max_prompt_length :]
                    else:
                        raise ValueError(f" {self.truncation_mode} truncation mode is Invalid!")

            # truncate response
            for answer_tokens in [chosen_tokens, rejected_tokens]:
                if len(answer_tokens["prompt_input_ids"]) + longer_response > self.max_length:
                    for i in ["input_ids", "attention_mask"]:
                        answer_tokens[i] = answer_tokens[i][: self.max_length - self.max_prompt_length]
        return batch
    
    #reference: https://github.com/huggingface/trl/blob/57d9a97394b4dd3aa5cd13dffdc99770b6326af2/trl/trainer/cpo_trainer.py#L369
